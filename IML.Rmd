...
they %>% filter(`What specific programming language do you use most often? - Selected Choice` %in% c("R","Python")) %>% 
  ...
  mutate(n = n / sum(n),
         perc = percent(n)) %>% 
   ggplot() + geom_col(aes(Language,n, fill = Language), stat = "identity", show.legend = FALSE) +
   geom_label(aes(x = Language, y = n - 0.05, label = percent(n)),
           # hjust=0, vjust=0, size = 4, colour = 'black',
            fontface = 'bold') +
    scale_fill_viridis(discrete = T, option = "D") +
  scale_y_continuous(labels = percent_format()) +
...
not_they   %>% filter(`What specific programming language do you use most often? - Selected Choice` %in% c("R","Python")) %>% 
  group_by(`What specific programming language do you use most often? - Selected Choice`) %>% count() %>% ungroup() %>% 
  rename("Language" = `What specific programming language do you use most often? - Selected Choice`) %>% 
  mutate(n = n / sum(n),
         perc = percent(n)) %>% 
